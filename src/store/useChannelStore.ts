...
export const useChannelStore = create<ChannelState>((set, get) => ({
  ...
  removeChannel: (channelId: number, type: 'group' | 'direct') =>
    set(state => ({
      groupChannels: type === 'group'
        ? state.groupChannels.filter(ch => ch.id !== channelId)
        : state.groupChannels,
      directChannels: type === 'direct'
        ? state.directChannels.filter(ch => ch.id !== channelId)
        : state.directChannels
    })),

  // New WebSocket channel management methods
  setCurrentChannelId: (channelId: string | null) => {
    console.log('ChannelStore: Setting current channel ID to:', channelId);
    set({ currentChannelId: channelId });
  },

  addJoinedChannel: (channelId: string) => {
    console.log('ChannelStore: Adding joined channel:', channelId);
    set(state => {
  ...
  removeJoinedChannel: (channelId: string) => {
    console.log('ChannelStore: Removing joined channel:', channelId);
    set(state => {
      const newJoinedChannels = new Set(state.joinedChannels);
  ...
  clearJoinedChannels: () => {
    console.log('ChannelStore: Clearing all joined channels');
    set({
      joinedChannels: new Set<string>(),
      currentChannelId: null
    });
